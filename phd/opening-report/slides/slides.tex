...
\begin{frame}
  ...
  \item 多发射/动态调度/乱序执行
  \item SIMD指令
    \begin{itemize}             % a table or figure here?
...
\subsection[并行可扩展性研究]{面向多核及众核处理器的并行优化技术研究}

\begin{frame}
  \frametitle{面向多核及众核处理器的并行优化技术研究}
  \begin{itemize}
  \item 问题描述
    \begin{itemize}
    \item 线程级并行度进一步提高，现有分块与调度策略需要调整
      ...
      \item 一维并行划分可扩展性差
      \end{itemize}
    \item 一些处理器不符合简单的inclusive cache存储层次结构假设
      \begin{itemize}
      \item 小容量exclusive LLC（FT-1500A）
      \item 众核处理器的互联cache（KNC/KNL）
      \end{itemize}
    \end{itemize}
  \item 研究现状
    \begin{itemize}
